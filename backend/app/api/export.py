...
those, so the written geometry is exactly what the user previewed). Writes operate
in place on the configured database, taking a backup before every write.

Errors map to status codes the frontend can act on:
* 422 — the staged edits / target schema are invalid (``ValidationError``).
* 409 — the target DB is busy (NINA open), read-only/unwritable (network share or
  sync replica), or the project has captured progress.
...
def edit_export(project_id: int, spec: ProjectSpec) -> ExportResult:
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=str(e))
    except (DatabaseBusyError, DatabaseReadOnlyError, ProgressError, EditNotAllowedError) as e:
        raise HTTPException(status_code=409, detail=str(e))
    except ExportError as e:
        raise HTTPException(status_code=400, detail=str(e))
